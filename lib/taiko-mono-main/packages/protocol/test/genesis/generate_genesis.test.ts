...
action("Generate Genesis", function () {
    ...
    describe("contracts can be called normally", function () {
        ...
        it("ERC20Upgradeable", async function () {
            const ERC20Upgradeable = new hre.ethers.Contract(
            ...
            expect(await ERC20Upgradeable.name()).to.be.equal(TOKEN_NAME);
            expect(await ERC20Upgradeable.symbol()).to.be.equal(TOKEN_SYMBOL);

            for (const seedAccount of seedAccounts) {
                const accountAddress = Object.keys(seedAccount)[0];

                expect(await ERC20Upgradeable.balanceOf(accountAddress)).to.be.equal(
                    PREMINT_SEED_ACCOUNT_BALANCE
                );
            }

            expect(await ERC20Upgradeable.totalSupply()).to.be.equal(
                seedAccounts.length * PREMINT_SEED_ACCOUNT_BALANCE
            );

            await expect(
                ERC20Upgradeable.transfer(ethers.Wallet.createRandom().address, 1)
            ).to.emit(ERC20Upgradeable, "Transfer");
        });
    });
